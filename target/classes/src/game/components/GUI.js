...
export default class GUI{
    ...
    initCameraHeight(){
        this.makeGuiRangeControl("camera height", (e) => {
            this.camera.changeHeight(e.target.value);
        }, 10, 80, this.camera.getPos().y)
    }

    initCameraXAngle(){
        this.makeGuiRangeControl("camera X angle", (e) => {
            this.camera.changeXAngle(e.target.value);
        }, -Math.PI/2, Math.PI/2, 0)
    }

    initChangeRootDistance(){
        this.makeGuiRangeControl("camera distance from player", (e) => {
            this.camera.changeRootDistance(parseFloat(e.target.value));
        }, 10, 50, 45)
    }

    initCameraYAngle(){
        this.makeGuiRangeControl("camera y angle", (e) => {
            this.camera.changeYAngle(e.target.value);
        }, -Math.PI/2, Math.PI/2, 0)
    }

    initchangeFov(){
        this.makeGuiRangeControl("camera fov", (e) => {
            this.camera.changeFov(e.target.value);
        }, 30, 90, 50)
    }

    initchangeLightIntensity(){
        this.makeGuiRangeControl("lights intensity", (e) => {
    ...
    makeGuiRangeControl(text, event, min, max, startVal){
        ...
        let range = $(`<input type='range' min="${min}" max="${max}" value="${startVal}" step="0.05"/>`)
        block.append(range);
